        ...
        <div class="widget-bg">
			...
			<div class="form-group">
                            <!--<select class="form-control week" name="week" style="color: #00ADEE;background: #F4F4F4;">
                                ...
                                <option value="<?php // echo $wrow['start_date'].'/'.$wrow['end_date']; ?>"  <?php //echo (date('Y-m-d')>=$wrow['start_date'] && date('Y-m-d')<= $wrow['end_date'] )?'selected':''; ?>>Week<?php //$i++; ?></option>
                                <?php //} } ?>
                            </select>-->
...
<div class="widget-holder widget-full-height widget-flex col-lg-6">
    <div class="widget-bg">
        <div class="widget-heading" style="display: block;">
                        ...
                        <div class="col-md-7" style="background: #8dc63f;padding: 10px;width: 300px;height: 42px;color: #fff !important;border-top-right-radius: 50px 20px;border-left: 3px solid #fff;"><h5 class="widget-title" style="color: #fff;">Weekly installation (In Number)</h5></div>
                        <div class="col-md-2"></div>
                        <div class="col-md-3">
			<div class="form-group">
                            <select class="form-control week" name="week" style="color: #00ADEE;background: #F4F4F4;">
        ...
        <div class="widget-body" style="width: 96%;height:200px;border: 2px solid;">
            <div class="mr-t-10 flex-1">
                <div style="max-height: 270px; height: 270px" id="graph-container4">
                     <canvas id="chartjs-4" height="200"></canvas>
                </div><p> </p>
            </div>
        </div>
			...
			<div class="form-group">
                            <select class="form-control year" name="year" style="color: #00ADEE;background: #F4F4F4;">
                                <?php for($i=2019;$i<=2020;$i++){ ?>
                                    <option value="<?php echo $i; ?>"  <?php echo (date("Y")==2019)?'selected':''; ?>><?php echo $i; ?></option>
                                    <?php } ?>
                            </select>
                        </div>
			</div>
...
$.post('<?=base_url();?>dashboard/yearly_app_info',{year:$(this).val()},function(data){
    ...
    "borderColor":"#5B9BD5","lineTension":1}
                ...
                "yAxes":[{scaleLabel: { display: true, labelString: "Installation" },"ticks":{'precision': 0,}}],
...
//Daily
			...
			var amountArray = [];
                            ...
                            <?php foreach($dailyInstal as $rowdata){    ?>
                                var day='<?php echo $rowdata['day'] ?>';
                                var dailydata='<?php echo $rowdata['dailydata'] ?>';
					labelsArray.push(day);
					amountArray.push(dailydata);
                            <?php } ?>
...
$.post('<?=base_url();?>dashboard/Dailyweekwise_app_info',{week:$(this).val()},function(data){
			...
			$('#chartjs-4').remove(); // this is my <canvas> element
			$('#graph-container4').append('<canvas id="chartjs-4"><canvas>');
     ...
     $(document).on('change','.month',function(){
        $.post('<?=base_url();?>dashboard/MonthwiseWeeklyOS_app_info',{month:$(this).val()},function(data){
			...
			$('#graph-container3').append('<canvas id="chartjs-3"><canvas>');
                    ...
                    "options":{
                            "scales":{
                                "yAxes":[{scaleLabel: { display: true, labelString: "Users" },"ticks":{'precision': 0,}}],
