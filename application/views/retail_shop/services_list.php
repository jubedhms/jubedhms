	...
	<div class="row page-title clearfix">
		...
		<!-- /.page-title-left -->
		  <div class="page-title-right d-none d-sm-inline-flex">
			...
			<a href="<?=base_url('services/addedit_service');?>" class="btn btn-info btn-sm">Add</a>
			<?php } if(checkModulePermission($MODULEID,'delete_btn')){ ?>
			<a href="javascript:void(0)" class="btn btn-danger btn-sm bulk_delete" id="<?=base_url('services/deleteData');?>" title="Delete selected rows">Delete All</a>
			<?php }?>
			</span>
		</div> 
	<!-- /.page-title-right -->
		...
		<div class="widget-body clearfix">
			...
			<tbody>
				...
				<tr>
					...
					<input type="checkbox" class="status" name="status[]" value="services.'^'.MD5($data->ID);?>" <?=($data->show_status=='1')?"checked":"";?> />
					<span class="toggle"></span></label></div>
					</td>
					<?php }?>
					<td>
                    <?php if(checkModulePermission($MODULEID,'edit_btn')){ ?>
						...
						<a href="<?=base_url('services/view_service');?>/<?=MD5($data->ID);?>" title="View product">
                            <img src="<?= base_url('assets/img/icon/view.png');?>"/>
                        </a> 
                     <?php }if(checkModulePermission($MODULEID,'edit_btn')){?>
					<a href="<?=base_url('services/addedit_service');?>/<?=MD5($data->ID);?>" title="Edit patient">
					<img src="<?= base_url('assets/img/icon/edit.png');?>"/>
					</a>
					<?php } if(checkModulePermission($MODULEID,'delete_btn')){ ?>
					<a href="javascript:void(0)" class="single_delete" id="services/deleteData/<?=md5($data->ID);?>" title="Delete patient">
