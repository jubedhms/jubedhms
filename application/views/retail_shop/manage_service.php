...
<!-- Page Title Area -->
	<div class="row page-title clearfix">
		...
		<div class="page-title-right d-none d-sm-inline-flex">
			...
			<a href="<?=base_url('service/addedit_service');?>" class="btn btn-info btn-sm">New</a>
			<?php } if(checkModulePermission($MODULEID,'delete_btn')){ ?>
			<a href="javascript:void(0)" class="btn btn-danger btn-sm bulk_delete" id="<?= base_url('service/deleteData');?>" title="Delete selected rows">Delete</a>
			<?php }?>
			</span>
		</div>
	<!-- /.page-title-right -->
		...
		<div class="widget-body clearfix">
			...
			<tbody>
				...
				<tr>
					...
					<input type="checkbox" class="status" name="status[]" value="<?= tblprefix("service").'^'.MD5($data->ID);?>" <?=($data->show_status=='1')?"checked":"";?> />
						 ...
						 <div class="dropdown">
							...
							<div class="dropdown-menu dropdown-menu-left action-dropdown">
								<?php if(checkModulePermission($MODULEID,'view_btn')){ ?>
									<a href="<?=base_url('service/view_service');?>/<?=MD5($data->ID);?>" class="dropdown-item btn btn-info  btn-xs btn-block action-btn min-btn">View</a>	
								<?php } if(checkModulePermission($MODULEID,'edit_btn')){ ?>
									<a href="<?=base_url('service/addedit_service');?>/<?=MD5($data->ID);?>" class="dropdown-item btn btn-success  btn-xs btn-block action-btn min-btn">Edit</a>
								<?php } if(checkModulePermission($MODULEID,'delete_btn')){ ?>
									<a href="javascript:void(0)" class="single_delete dropdown-item btn btn-danger btn-xs btn-block action-btn min-btn" id="service/deleteData/<?=md5($data->ID);?>">Delete</a>
								<?php }?>
							</div>
						</div>
					</td>
