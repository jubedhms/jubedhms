...
class Vaccine_appointment_model extends CI_Model {
    ...
    public function get_data($prn){
        extract($_GET);
        if($prn!=''){
            $this ->db->select('VA.prn,VA.parent_prn,VA.ID as appointment_id,VA.his_appointment_number,VA.booking_date,VA.booking_time,VA.appointment_date,VA.appointment_time,VA.description,VA.is_deleted');
			...
			$this ->db->select('V.vaccine_name,DO.dose_name');
			...
			$this->db->join("vaccines as V",'V.ID=VA.vaccine_id','LEFT');
			$this->db->join("dose_schedule as DO",'DO.ID=VA.dose_id','LEFT');
			
            if(isset($is_parent) && $is_parent==true){    
                $this->db->group_start();
				$this->db->where('VA.prn',$prn);
                $this->db->or_where('VA.parent_prn',$prn);
				$this->db->group_end();
            }else{
				$this->db->where('VA.prn',$prn);
			}

            if(isset($from_date) && $from_date!='')$this->db->where('VA.appointment_date >=',$from_date);
            if(isset($to_date) && $to_date!='')$this->db->where('VA.appointment_date <=',$to_date);
            if(isset($is_deleted) && $is_deleted==false)$this->db->where('VA.is_deleted=',0);

            if(isset($limit) && $limit!='' && isset($start) && $start!=''){
                $this->db->limit($limit, $start);
            }

            $this->db->order_by("VA.appointment_date DESC,VA.appointment_time ASC");
    ...
    public function add_appointment(){
        ...
        $insertData=array(
            ...
            if($appointment_id){ 
                $title="Vaccine appointment confirmation"; 
                $description="Your vaccination appointment with the doctor ".$doctor_name." on Appointment Date ".$appointment_date." ". date("h:i:s A", strtotime($appointment_time))." at ".$hospital_location_name." has been confirmed. Please present at hospital before 15 mins for check in purpose.";
				$title_vi="Vắc xin cuộc hẹn xác nhận"; 
                $description_vi="Lịch hẹn tiêm ngừa vắc xin của bạn với bác sĩ ".$doctor_name." vào ngày ".$appointment_date." ". date("h:i:s A", strtotime($appointment_time))." tại ".$hospital_location_name." đã được xác nhận. Bạn vui lòng có mặt trước 15 phút đẻ chuẩn bị tiêm ngừa.";
				
                $notficationData=[
                   'username'=>$username,
                    'parent_username'=>$parent_username,
                    'module_name'=> 'vaccine_appointment',
    ...
    public function loadDataById($ID=''){
        ...
        $this ->db->select('VA.ID as appointment_id,PA.his_appointment_number,"3" as appointment_type_id,VA.prn as patient_prn,VA.parent_prn,HL.code as hospital_location_code,VA.doctor_speciality_code,VA.doctor_mcr,VA.vaccine_id,VA.dose_id,VA.booking_date,VA.booking_time,VA.appointment_date,VA.appointment_time,VA.description');
	...
	public function delete_appointment($appointment_id=''){
        ...
        if($result){
			...
			$title_vi="Bổ nhiệm Hủy xác nhận"; 
