...
class Doctor_model extends CI_Model {
    ...
    public function get_data_time_schedule($doctor_mcr='',$DoctorAvailabilityData=[]){ 
        extract($_GET);
        $doctor_availability=[];
		$crr_date=date("Y-m-d");
        $crr_time=date("H:i:s");
        ...
        if($doctor_mcr!=''){ 
			if($DoctorAvailabilityData){
				$key=0;
				foreach($DoctorAvailabilityData as $value){
				...
				//echo "<pre>";print_r($doctor_availability);die;		
				}
			}
        }
		//end
