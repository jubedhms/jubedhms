...
class Vaccine_model extends CI_Model {
    ...
    public function getVaccine($language){
        if($language=='en'){
        $query=$this->db->select('ID,vaccine_name')->from($this->main_table)->where('is_deleted',0)->get();
        }else{
            $query=$this->db->select('ID,vaccine_name_vi as vaccine_name')->from($this->main_table)->where('is_deleted',0)->get();
        }
        if($query->num_rows()>0){    
    ...
    public function getDoseByVaccineId($vaccine_id){
        $query=$this->db->select('ID,vaccine_id,dose_name,from_month,to_month')->from('dose_schedule')->where('is_deleted',0)->where('vaccine_id',$vaccine_id)->get();
        if($query->num_rows()>0){    
            return $query->result_array();
        }else{
            return false;  
    ...
    public function getpatientDOB_username($username){
        $query=$this->db->select('dob')->from('patient')->where('is_deleted',0)->where('username',$username)->get();
        if($query->num_rows()>0){    
            return $query->row()->dob;
        }else{
            return false;  
    ...
    public function getsub_patientDOB_uname($username,$parent_username){
        $query=$this->db->select('dob')->from('sub_patient')->where('is_deleted',0)->where('parent_username',$parent_username)->where('username',$username)->get();
        if($query->num_rows()>0){    
            return $query->row()->dob;
        }else{
            return false;  
    ...
    public function getRemaningDose($months,$limit){
        ...
        $query=$this->db->query("SELECT V.ID, V.vaccine_name,V.vaccine_origin,V.disease_name,V.vaccine_short_name,V.available_status, D.`from_month`,D.`to_month`,D.ID as dose_id, D.dose_name FROM `hms_dose_schedule` as D "
    ...
    public function getpatient($prn){
        $query=$this->db->select('P.ID,P.first_name,P.middle_name,P.last_name,P.dob,P.gender,PD.image,P.username')->from('patient as P')->join('patient_documents as PD','PD.patient_id=P.ID')->where('P.is_deleted',0)->where('P.prn',$prn)->get();
        //echo $this->db->last_query();die;
        if($query->num_rows()>0){    
            return $query->row();
        }else{
    ...
    public function getpatient_username($prn){
        $query=$this->db->select('P.username as username')->from('patient as P')->where('P.is_deleted',0)->where('P.prn',$prn)->get();
        if($query->num_rows()>0){    
    ...
    function getUserDueDatedetails($username){
        $query=$this->db->select('ID,username,method,mperiod,last_mperiad_date,due_date,due_days')->from('patient_duedates')->where('is_deleted',0)->where('username',$username)->get();
        if($query->num_rows()>0){
            return $query->row();
        }else{
            return false;  
    ...
    function savekick($username,$week,$kick_count){
        $querydata=$this->db->get_where('patient',array('username'=>$username))->row();
        if(@count($querydata)>0){
    ...
    function Updatekick($username,$kick_count){
        $querydata=$this->db->get_where('patient',array('username'=>$username))->row();
        if(@count($querydata)>0){
            $maxid=$this->db->select('MAX(ID) as maxid')->from('kick_counts')->where(array('date'=>date('Y-m-d'),'username'=>$username))->get()->row()->maxid;
            if($maxid){
                    $end_time=date('H:i:s');
                    $data=array( "end_time"=>$end_time,"kick_count"=>$kick_count);
                        if($this->db->update('kick_counts',$data,array('username'=>$username,'ID'=>$maxid))){
                            return true;
                        }else{
                            return false;
                        }
    ...
    function getkickData($username){
        $querydata=$this->db->get_where('patient',array('username'=>$username))->row();
        if(@count($querydata)>0){
                $data=$this->db->get_where('kick_counts',array('username'=>$username,'date'=>date('Y-m-d')))->result();
                    return $data;
                }else{
                    return false;
                }
    ...
    function getkickDataBydatePRN($username,$date,$week){
        $querydata=$this->db->get_where('patient',array('username'=>$username))->row();//If patient exist
        if(@count($querydata)>0){
            if($date){
                $search=array(
                    'username'=>$username,
    ...
    function allKickedDate($username,$week){
        $due_date=$this->getduedate($username);
	...
	function SaveSchedule(){
        ...
        extract($_POST);
            $data=array("username"=>$username,
                        ...
                        "time"=>isset($time)?$time:$crr_time,
                        "week"=>isset($week)?$week:'',
                        "date"=>$crr_date,
                        "show_status"=>1
            );
        if($this->db->insert('patient_schedules',$data)){
            return true;
        }else{
            return false;
        }
    ...
    function getScheduleData($username,$week){
        $query=$this->db->select('username,activity,time,week,date')->from('patient_schedules')->where(array('username'=>$username,'week'=>$week,'is_deleted'=>0))->get();
        if($query->num_rows()>0){    
            return $query->result();
        }else{
            return false;  
