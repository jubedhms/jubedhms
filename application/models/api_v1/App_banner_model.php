			...
			foreach($result as $key=>$value){
				if($value->module_name!='notification'){
					...
					if($value->module_name=='awareness'){
						$this ->db->select('name,awareness_image as image,description,file_type,category,ages,relationship,province,city,district,gender');
						...
						$this ->db->select('name,pregnancy_period_image as image,description,file_type, "" AS category');	
					}else if($value->module_name=='special_offer'){
						$this ->db->select('name,image,description,"1" AS file_type, "" AS category');
					...
					if($res && isset($value->name) && $value->name!='' && isset($value->text) && $value->text!=''){
						...
						$response[]=[
								...
								'category'=> (isset($res->category) && $res->category!='')?explode(",",$res->category):[],
								'short_by'=>$key,
							];
					}
				}
