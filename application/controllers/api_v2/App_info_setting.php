...
class App_info_setting extends  REST_Controller {
	/**
	...
	*/
    ...
    public function __construct() {
		parent::__construct();
		$this->load->database();
		$this->load->model('api_v1/app_info_setting_model');
	}
	...
	/*
	* Method name: app_version
	* Desc: get version of app status(true for accepted, false for denied)
	*/
	...
	{ 
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){	
					$response=$this->app_info_setting_model->get_app_version();	
					if($response){
		...
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){	
					...
					$response=$this->app_info_setting_model->get_data($language);	
					if($response){
						$json = array("status" => 1, "message" => "Ok", "data"=> $response);
					}else{
						$json = array("status" => 0, "message" => "Somthing went wrong. Please try again later");
