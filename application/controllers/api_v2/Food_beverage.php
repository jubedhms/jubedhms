...
class Food_beverage extends  REST_Controller {
	/**
	...
	*/
    ...
    public function __construct() {
		parent::__construct();
		$this->load->database();
		$this->load->model('api_v1/food_beverage_model');
	}
	...
	{ 
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){
					$response=$this->food_beverage_model->get_data();	
					if($response){
		...
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){	
					...
					if($item_code!=''){
						$response=$this->food_beverage_model->loadDataById($item_code);	
						if($response){
							$json = array("status" => 1, "message" => "Ok", "data"=> $response);
						}else{
							$json = array("status" => 0, "message" => "Somthing went wrong. Please try again later");
