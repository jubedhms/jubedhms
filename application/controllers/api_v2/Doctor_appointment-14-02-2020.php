...
class Doctor_appointment extends  REST_Controller {
	/**
	...
	*/
    ...
    public function __construct() {
		parent::__construct();
		$this->load->database();
		$this->load->model('api_v1/doctor_appointment_model');
	}
		...
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){	
					...
					if($prn!=''){
						$response=$this->doctor_appointment_model->get_data($prn);	
						if($response){
		...
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){	
					...
					if($appointment_id!=''){
						$response=$this->doctor_appointment_model->loadDataById($appointment_id);	
						if($response){
							$json = array("status" => 1, "message" => "Ok", "data"=> $response);
						}else{
							$json = array("status" => 0, "message" => "No data found");
		...
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){	
					...
					if($doctor_mcr!=''){
						$response=$this->doctor_appointment_model->get_data_time_schedule($doctor_mcr);	
						if($response){
							$json = array("status" => 1, "message" => "Ok", "data"=> $response);
						}else{
						...
						$json = array("status" => 0, "message" => "Doctor ID has been empty.");
					}
				}else{
					$json = array("status" => 0, "message" => "Token has been not matched");
				}	
	...
	{ 
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			if(isset($_POST)){
				...
				if($permission==true){
					extract($_POST);
					if(isset($prn) && isset($appointment_type_id) && isset($hospital_location_code) && isset($doctor_speciality_code) && isset($doctor_mcr) && isset($appointment_date) && isset( $appointment_time) && $prn!='' && $appointment_type_id!='' && $hospital_location_code!='' && $doctor_speciality_code!='' && $doctor_mcr!='' && $appointment_date!='' && $appointment_time!=''){
						$hospital_location_name=$this->doctor_appointment_model->hospitalLocationNameByCode($hospital_location_code);
						$doctor_name=$this->doctor_appointment_model->doctor_name($doctor_mcr);	
						$_POST['hospital_location_name']=$hospital_location_name;
						$_POST['doctor_name']=$doctor_name;

						$appointment_id=$this->doctor_appointment_model->add_appointment();	
						if($appointment_id){
							...
							if($message_type=='sms'){
								...
								$message='<div style="text-align:center;padding-bottom:20px;"><h2>Doctor Appointment Confirmed</h2></div><p>Your appointment with the doctor '.$doctor_name.' go to '. $appointment_date.' '.date("h:i:s A", strtotime ($appointment_time)).' at '. $hospital_location_name.' has been confirmed. Please arrive 20 minutes before the scheduled time. If you need further assistance, please contact Customer Service: 19006765.</p>';
								$template_id='1';
		...
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			if(isset($_POST)){
				...
				if($permission==true){
					extract($_POST);
					if(isset($prn) && isset($appointment_type_id)  && isset($hospital_location_code) && isset($doctor_speciality_code) && isset($doctor_mcr) && isset($appointment_date) && isset( $appointment_time) && $prn!='' && $appointment_type_id!='' && $hospital_location_code !='' && $doctor_speciality_code!='' && $doctor_mcr!='' && $appointment_date!='' && $appointment_time!=''){
						$appointment_id=$this->doctor_appointment_model->update_appointment();	
						if($appointment_id){
		...
		if($_SERVER['REQUEST_METHOD'] == 'DELETE'){
			...
			if(isset($_GET)){
				...
				if($permission==true){	
					$appointment_id= isset($_GET['appointment_id']) ?($_GET['appointment_id']) : "";
					if($appointment_id!=''){
						$response=$this->doctor_appointment_model->delete_appointment($appointment_id);
						if($response){
							...
							if($message_type=='sms'){
								...
								$subject="Doctor Appointment Canceled";
								$message='<div style="text-align:center;padding-bottom:20px;"><h2>Doctor Appointment Confirmed</h2></div><p>Your doctor appointment with the doctor '.$doctor_name.' go to '. $appointment_date.' '.date("h:i:s A", strtotime ($appointment_time)).' at '. $hospital_location_name.' has been canceled. If you need further assistance, please contact Customer Service: 19006765.</p>';
								$template_id='1';
								$response=$this->genrate_email($username,$subject,$message,$template_id);
							}
						}else{
