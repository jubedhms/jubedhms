...
class Province extends  REST_Controller {
	...
	*/
    ...
    public function __construct() {
		parent::__construct();
		$this->load->database();
		$this->load->model('api_v1/provinces_model');
	}
	...
	*/
	...
	{ 
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if($permission==true){
				$country_code= isset($_GET['country_code']) ?($_GET['country_code']) : "241";
				$province_code= isset($_GET['province_code']) ?($_GET['province_code']) : "";
				$response= $this->provinces_model->getData($country_code, $province_code);
				if($response){
