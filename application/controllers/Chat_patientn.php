...
class Chat_patientn extends MY_Controller {
	...
	public function index(){
		...
		$data['mode']=$this->mode;
		$details=$this->chat_modeln->getDataPatient($is_active='');
		$data['details']=$details;
		$data['MODULEID']=$this->MODULEID;
		$data['mode']="Manage";
		$data['heading']=$this->heading;
		$this->layout('chatbot_patientn',$data);
	}
	...
	public function openChat(){
           ...
           //die; if old message count is greater than 6 then new some question will not come like name,DOB,address.
            if($oldMsgCount>10 && $lastChatTime<= $compareTime){
             ...
             //$oldMsgCount=@count($arrdata2);//die;
...
//print_r($data['option']);die;
            $this->load->view('includes/patient_chatn',$data);
        }
	...
	public function revert_from_patient(){
            ...
            //$this->chat_modeln->SaveUserChats($data);
            $this->chat_modeln->updateChattimePatient($username);
            $data=$this->chat_modeln->UserChats($username);
            if(!empty($data)) {
                $data['msgg']=$data;
            }
            if($this->chat_modeln->lastMail($username)){ //Difference in mail of 12 hours
                //$this->sendMail();
            }
            $this->load->view('includes/patient_chatn',$data);
            //to mail if the not asnwered question is greater than 10 to support and admin to alert;
        ...
        public function chatBotOption(){
            ...
            $advisory_option='';
            $username=$this->input->post('username');//patient name
            $messageall=explode('^',$this->input->post('message'));
    ...
    private function sendMail(){
            $data=array();
            $dataquestion=$this->chat_modeln->totalQueryNotAnswered();
            $data['data']=$dataquestion;
            if(@count($dataquestion)>3){
		$config = Array(
		'protocol' => 'smtp',
		'smtp_host' => 'ssl://smtp.gmail.com',
		'smtp_port' => 465,
		'smtp_user' => 'jutest90@gmail.com', // change it to yours
		'smtp_pass' => 'bdyomlzbtbrpxrsc', // change it to yours
		'mailtype' => 'html',
		'starttls' => true,
		'newline' => "\r\n",
		'charset' => 'utf-8',
		); 
		//$message = 'Hi I am testing it for a long time thank you.';
		$this->load->library('email', $config);
		$this->email->initialize($config);
		//$this->email->set_newline("\r\n");
		$this->email->from('jubed.alam1990@gmail.com'); // change it to yours
		$this->email->to('jubed.alam1990@gmail.com');// change it to yours
		//$this->email->to('thaonguyen@ashavi.com');// change it to yours
		//$this->email->to('thuydinh@ashavi.com');// change it to yours
		//$this->email->subject('All user query that is not answered.');
		$this->email->subject('All user queries have not been answered.');
		$this->email->message($this->load->view('mail/NotAnsweredQuery.php', $data, TRUE));	
		...
		if($this->email->send()){
                    ...
                    $this->db->insert('mail_sent_details',$data);
        ...
        /*Mail is only for all user queries not for an single user.
        Mail will be send after the 12 hour everyday when any user chat.
        If the message is greater than three, message will be send.
        Customer care executive have to answer through chat to that user.
        History question will be updated from manage page. There are functionality of add,edit and delete.
        */
