...
class App_banner_setting extends  REST_Controller {
	/**
	...
	*/
    ...
    public function __construct() {
		parent::__construct();
		$this->load->database();
		$this->load->model('api_v1/app_banner_setting_model');
	}
		...
		if($_SERVER['REQUEST_METHOD'] == "GET"){
			...
			if(isset($_GET)){
				...
				if($permission==true){
					...
					if($module_name!=''){
						$response=$this->app_banner_setting_model->get_data();	
						if($response){
...
/*
	...
	* Desc: get list status(true for accepted, false for denied)
	*/
	public function get_home_list_get()
	{ 
		if($_SERVER['REQUEST_METHOD'] == "GET"){
        	// Get data
			if(isset($_GET)){
				...
				$permission=$this->matchAppToken($token);
				if($permission==true){	
					$response=$this->app_banner_setting_model->get_home_banner_data();	
					if($response){
						$json = array("status" => 1, "message" => "Ok", "data"=> $response);
					}else{
						$json = array("status" => 0, "message" => "Somthing went wrong. Please try again later");
