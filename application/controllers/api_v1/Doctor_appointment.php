...
class Doctor_appointment extends  REST_Controller {
	...
	*/
	public function add_appointment_post(){
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			if(isset($_POST)){
				...
				if($permission==true){
					extract($_POST);
					if(isset($prn) && isset($appointment_type_id) && isset($slot_number) && isset($hospital_location_code) && isset($doctor_speciality_code) && isset($doctor_mcr) && isset($appointment_date) && isset( $appointment_time) && $prn!='' && $appointment_type_id!='' && $slot_number!='' && $hospital_location_code!='' && $doctor_speciality_code!='' && $doctor_mcr!='' && $appointment_date!='' && $appointment_time!=''){
						$case_type=($appointment_type_id=='1')?'NC':'FU';	
	...
	{ 
		...
		extract($_POST);
		$hospital_location_name=$his_response["Clinic"]; //$this->doctor_appointment_model->hospitalLocationNameByCode($hospital_location_code);
		$his_appointment_number=$his_response['AppointmentNumber'];
		
		$doctor_name=$his_response['DoctorName'];//$this->doctor_appointment_model->doctor_name($doctor_mcr);	
		...
		$response=$this->doctor_appointment_model->add_appointment();	
		...
		if($response){
			...
			if($message_type=='sms'){
				$message="Lịch hẹn khám của bạn với bác sĩ ".$doctor_name." vào ngày ".$appointment_date." ". date("h:i:s A", strtotime($appointment_time))." tại ".$hospital_location_name." đã được xác nhận. Bạn vui lòng có mặt trước 15 phút đẻ chuẩn bị tiêm ngừa.";
				$response=$this->genrate_sms($username,$message);
			}else{
				$subject="Your appointment has been confirmed";
				$message='<div style="text-align:center;padding-bottom:20px;"><h2>Appointment confirmation.</h2></div><p>Your appointment with the doctor '.$doctor_name.' on Appointment Date '. $appointment_date.' '.date("h:i:s A", strtotime ($appointment_time)).' at '. $hospital_location_name.' has been confirmed. Please present at hospital before 15 mins for check in purpose.</p>';
				$template_id='1';
