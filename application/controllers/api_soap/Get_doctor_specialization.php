...
class Get_doctor_specialization extends CI_Controller {
	public function __construct(){
        parent::__construct();
		$this->load->model("api_token_model");
		$this->load->model("api_soap/get_doctor_specialization_model");
	}
	...
	public function index(){ 
		...
		if($details && isset($details->token_number) && $details->token_number!='' && isset($details->company_code) && $details->company_code!=''){
			try {
				$wsdl = base_url("WSDL/WebMethodGetSpecialtyData.xml");
				$wsdlLocation=$details->url."hpih_uat/web_services/APPOINTMENT/GetSpecialtyData.cfc";
				...
				//print_r($params);die();			
				$result = $client->GetSpecialtyData($params);
				...
				if($result && isset($result->return)){
					...
					if($response){
						...
						$return=$this->get_doctor_specialization_model->add_data($response);
